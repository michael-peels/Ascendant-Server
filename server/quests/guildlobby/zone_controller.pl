sub EVENT_SPAWN {
    quest::settimer("af_heckle", 1);  # Fire immediately, then timer takes over
}
...
sub EVENT_TIMER {
    if ($timer eq "af_heckle") {
        ...
        if (!plugin::AprilFools_Enabled()) {
            quest::settimer("af_heckle", 240 + int(rand(361)));
            return;
        }
        ...
        quest::settimer("af_heckle", 240 + int(rand(361)));  # 4-10 minutes
    }
}
