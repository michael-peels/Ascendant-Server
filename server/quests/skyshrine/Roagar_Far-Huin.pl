...
sub EVENT_ITEM {
   if ($faction == 1) { #req ally CoV
         ...
         plugin::velious_armor_tier_reward(55324); # Item: Coif of Fire's Fury
      }
      elsif (plugin::check_handin(\%itemcount, 25814 => 3, 24963 => 1)) { #tunic
         plugin::velious_armor_tier_reward(55325); # Item: Tunic of Fire's Fury
      }
      elsif (plugin::check_handin(\%itemcount, 25821 => 3, 24965 => 1)) { #sleeves
         plugin::velious_armor_tier_reward(55326); # Item: Sleeves of Fire's Fury
      }
      elsif (plugin::check_handin(\%itemcount, 25838 => 3,24967  => 1)) { #bracer
         plugin::velious_armor_tier_reward(55327); # Item: Bracer of Fire's Fury
      }
      elsif (plugin::check_handin(\%itemcount, 25832 => 3, 24969 => 1)) { #guantlet
         plugin::velious_armor_tier_reward(55328); # Item: Gauntlets of Fire's Fury
      }
      elsif (plugin::check_handin(\%itemcount, 25825 => 3, 24964 => 1)) { #leggings
         plugin::velious_armor_tier_reward(55329); # Item: Leggings of Fire's Fury
      }
      elsif (plugin::check_handin(\%itemcount, 25833 => 3, 24966 => 1)) { #boots
         plugin::velious_armor_tier_reward(55330); # Item: Boots of Fire's Fury
      }
      else {
