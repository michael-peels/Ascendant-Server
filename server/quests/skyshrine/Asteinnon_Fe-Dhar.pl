...
sub EVENT_ITEM {
  if ($faction == 1) { #req ally CoV 
    if (plugin::check_handin(\%itemcount, 25841 => 3, 24968 => 1)) { #cap
      plugin::velious_armor_tier_reward(31105); # Item: Wolf Caller's Helm
    }
    elsif (plugin::check_handin(\%itemcount, 25805 => 3, 24963 => 1)) { #bp
      plugin::velious_armor_tier_reward(31106); # Item: Wolf Caller's Breastplate
    }
    elsif (plugin::check_handin(\%itemcount, 25815 => 3, 24965 => 1)) { #arm
      plugin::velious_armor_tier_reward(31107); # Item: Wolf Caller's Vambraces
    }
    elsif (plugin::check_handin(\%itemcount, 25836 => 3,24967  => 1)) { #bracer
      plugin::velious_armor_tier_reward(31108); # Item: Wolf Caller's Bracer
    }
    elsif (plugin::check_handin(\%itemcount, 25840 => 3, 24969 => 1)) { #glove
      plugin::velious_armor_tier_reward(31109); # Item: Wolf Caller's Gauntlets
    }
    elsif (plugin::check_handin(\%itemcount, 25827 => 3, 24964 => 1)) { #leg
      plugin::velious_armor_tier_reward(31110); # Item: Wolf Caller's Greaves
    }
    elsif (plugin::check_handin(\%itemcount, 25838 => 3, 24966 => 1)) { #boots
      plugin::velious_armor_tier_reward(31111); # Item: Wolf Caller's Boots
    }
    else {
