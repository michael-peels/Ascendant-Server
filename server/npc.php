...
switch ($action) {
  ...
  case 12: // Search for primary faction
    check_authorization();
    $body = new Template("templates/npc/npc_faction_id.search.tmpl.php");
    $body->set('currzone', $z);
    $body->set('currzoneid', $zoneid);
    $body->set('npcid', $npcid);
    break;
  case 13: // Primary faction search results
    check_authorization();
    $body = new Template("templates/npc/npc_faction_id.searchresults.tmpl.php");
    $body->set('currzone', $z);
    $body->set('currzoneid', $zoneid);
    $body->set('npcid', $npcid);
...
function npc_info() {
  ...
  $result['factionname'] = '';
  $result['npc_faction_id'] = '';
  $result['npc_faction_idname'] = '';
  $result['faction_hits'] = '';
  ...
  if ($factionid != 0) {
    ...
    $result['npc_faction_id'] = $result2['npc_faction_id'];
    $result['npc_faction_idname'] = get_faction_name($result2['npc_faction_id']);
...
function search_npc_faction_ids_primary($search) {
  global $mysql_content_db;
  $query = "SELECT nf.id, nf.name, fl.name AS npc_faction_id FROM npc_faction nf 
            INNER JOIN faction_list fl ON fl.id=nf.npc_faction_id
...
function update_primary_faction() {
  ...
  $id = get_npc_faction_id($npcid);
  $fid = $_GET['fid'];
  $query = "UPDATE npc_faction SET npc_faction_id=$fid WHERE id=$id";
  $mysql_content_db->query_no_result($query);
}
